...
// Colors
const PALETTE = {
  sky: ["#0a0a1a", "#0d1b3e", "#1a1a2e"],
  road: "#1c1c2e",
  roadEdge: "#2a2a4a",
  lane: "#f5c518",
  grass: "#0b1a0b",
  playerCar: { body: "#e74c3c", roof: "#c0392b", window: "#85c1e9", wheel: "#1a1a1a" },
  enemyCars: [
...
// ─── Drawing helpers ──────────────────────────────────────────────────────────
function drawCar(ctx, x, y, w, h, colors, isPlayer = false, flip = false) {
  ctx.save();
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function InfiniteRacer() {
  ...
  useEffect(() => {
    ...
    const spawnEnemy = (s) => {
      ...
      const colorSet = PALETTE.enemyCars[randInt(0, PALETTE.enemyCars.length - 1)];
      s.enemies.push({
    ...
    const loop = () => {
      ...
      ctx.save();
      if (s.shake > 0) {
        ctx.translate(rand(-s.shake * 0.6, s.shake * 0.6), rand(-s.shake * 0.3, s.shake * 0.3));
      }
      ...
      {screen === "menu" && (
        <div style={{ textAlign: "center", zIndex: 10 }}>
          ...
          <button onClick={startGame} style={{
            padding: "18px 64px", fontSize: 22, fontWeight: 700,
            fontFamily: "'Courier New', monospace",
